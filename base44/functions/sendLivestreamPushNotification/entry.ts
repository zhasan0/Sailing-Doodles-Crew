...
Deno.serve(async (req) => {
  try {
    ...
    const streams = await base44.asServiceRole.entities.Livestream.filter({ id: livestream_id });
    if (!streams[0]) return Response.json({ error: 'Livestream not found' }, { status: 404 });
    ...
    const iosTokens = allTokens.filter(t => t.device_type === 'ios');

    let sent = 0;
    for (const t of iosTokens) {
      const ok = await sendAPNS(jwt, t.token, '🔴 Going Live!', stream.title, { type: 'livestream', livestream_id });
      if (ok) sent++;
    }
